...
DEFAULT_EXCLUDE_PDF_SUBSTRINGS = [
    "logo/", "PointSpreadFunctions/", "PSF_", "PSFs_", "PSF-", "PSF.", 
    "/graphics/", "workflow-", "Glossary.pdf", "column-mapping.pdf"
]
...
def clone_repositories(config_path: str, base_path: str = "knowledge_base/raw", dry_run: bool = False, category: str = None, force_update: bool = False) -> dict:
    ...
    with open(config_path, "r") as f:
        config = yaml.safe_load(f)
    categories = [category] if category else list(config.keys())
    failures = {'failed_clones': [], 'failed_updates': [], 'skipped_existing': [], 'successful_clones': [], 'successful_updates': []}
    for cat in categories:
        ...
        for repo in repos:
            ...
            if dest_dir.exists():
                ...
                try:
                    subprocess.run(["git", "clone", repo_url, str(dest_dir)], check=True, capture_output=True, text=True)
...
def build_txtai_index(config_path: str, articles_config_path: str = None, base_path: str = "knowledge_base/raw", embeddings_path: str = "knowledge_base/embeddings", dry_run: bool = False, category: str = None) -> dict:
    ...
    try:
        ...
        return {'failed_text_files': [], 'failed_pdf_files': [], 'failed_notebook_conversions': [], 'successful_text_files': 0, 'successful_pdf_files': 0, 'successful_notebook_conversions': 0, 'skipped_repositories': [], 'skipped_articles': []}
    ...
    general_embeddings = Embeddings({"path": "sentence-transformers/all-MiniLM-L6-v2", "content": True, "backend": "faiss"})
    ...
    failures = {'failed_text_files': [], 'failed_pdf_files': [], 'failed_notebook_conversions': [], 'successful_text_files': 0, 'successful_pdf_files': 0, 'successful_notebook_conversions': 0, 'skipped_repositories': [], 'skipped_articles': []}
    for cat in categories:
        repos = config.get(cat)
        if not isinstance(repos, list):
            continue
            ...
            for pf in pdf_files:
                try:
                    if is_excluded_pdf(pf):
                        pdf_status[str(pf)] = {"status": "excluded", "reason": "pattern", "size": pf.stat().st_size if pf.exists() else 0}
                        continue
            ...
            if tika_ready or pdf_fallback_available:
                try:
                    ...
                    if success and content:
                        ...
                        documents.append((doc_id, full_content))
                        failures['successful_pdf_files'] += 1
                        pdf_status[doc_id] = {"status": "indexed", "size": pdf_file.stat().st_size if pdf_file.exists() else 0, "chars": len(content)}
                    else:
...
if __name__ == "__main__":
    ...
    if 'build_pipeline' not in globals():
        def print_pipeline_summary(all_failures: dict, dry_run: bool = False) -> None:
            ...
            if repo_failures:
                logger.info("\n📁 REPOSITORIES:")
                if repo_failures.get('successful_clones'): logger.info(f"  ✅ Successfully cloned: {len(repo_failures['successful_clones'])}")
                if repo_failures.get('successful_updates'): logger.info(f"  🔄 Successfully updated: {len(repo_failures['successful_updates'])}")
                if repo_failures.get('skipped_existing'): logger.info(f"  ⏭️  Skipped (already exists): {len(repo_failures['skipped_existing'])}")
                if repo_failures.get('failed_clones'): logger.info(f"  ❌ Failed to clone: {len(repo_failures['failed_clones'])}")
                if repo_failures.get('failed_updates'): logger.info(f"  ❌ Failed to update: {len(repo_failures['failed_updates'])}")
            article_failures = all_failures.get('articles', {})
            if article_failures:
                logger.info("\n📚 PDF ARTICLES:")
                if article_failures.get('successful_downloads'): logger.info(f"  ✅ Successfully downloaded: {len(article_failures['successful_downloads'])}")
                if article_failures.get('skipped_existing'): logger.info(f"  ⏭️  Skipped (already exists): {len(article_failures['skipped_existing'])}")
                if article_failures.get('failed_downloads'): logger.info(f"  ❌ Failed to download: {len(article_failures['failed_downloads'])}")
            ...
            if indexing_failures:
                ...
                if indexing_failures.get('skipped_articles'): logger.info(f"  ⏭️  Skipped articles: {len(indexing_failures['skipped_articles'])}")
                if indexing_failures.get('failed_pdf_files'): logger.info(f"  ❌ Failed PDF files: {len(indexing_failures['failed_pdf_files'])}")
            total_failures = sum(len(v) for k,v in indexing_failures.items() if k.startswith('failed')) + len(article_failures.get('failed_downloads', []))
            logger.info("\n" + "=" * 60)
            if total_failures == 0:
        ...
        def build_pipeline(config_path: str, articles_config_path: str = None, base_path: str = "knowledge_base/raw", embeddings_path: str = "knowledge_base/embeddings", dry_run: bool = False, category: str = None, force_update: bool = False, dirty: bool = False) -> None:
            ...
            if articles_config_path and os.path.exists(articles_config_path):
                all_failures['articles'] = download_pdf_articles(articles_config_path, base_path, dry_run, category, force_update)
            all_failures['indexing'] = build_txtai_index(config_path, articles_config_path, base_path, embeddings_path, dry_run, category)
